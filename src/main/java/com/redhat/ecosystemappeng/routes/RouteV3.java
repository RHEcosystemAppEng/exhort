...
public class RouteV3 extends EndpointRouteBuilder {
    ...
    public void configure() {
        ...
        rest("/api/v3/")
                ...
                .param().name("pkgManager").type(RestParamType.body).required(true)
                .endParam()
                .param().name("provider").type(RestParamType.body)
                .required(true).endParam()
                .to("direct:componentAnalysis")
                .post("/dependency-analysis/{pkgManager}/{provider}")
                .param().name("pkgManager").type(RestParamType.path).required(true)
                .endParam()
                .param().name("provider").type(RestParamType.path)
                .required(true).endParam()
