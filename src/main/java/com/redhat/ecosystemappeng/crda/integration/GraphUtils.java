...
public class GraphUtils {
  ...
  private DependencyTree fromSbom(InputStream file) {
    try {
      ...
      if (bom.getComponents() != null) {
        componentPurls.putAll(
            bom.getComponents().stream()
                ...
                .collect(Collectors.toMap(Component::getBomRef, c -> fromComponent(c))));
        List<String> types =
      ...
      if (rootComponent.isPresent()) {
        treeBuilder.root(fromComponent(rootComponent.get()));
      } else { // rootless SBOM
  ...
  private boolean requiresDecoding(String pkgManager) {
    return Constants.PYPI_PKG_MANAGER.equals(pkgManager);
  }

  private PackageRef fromComponent(Component component) {
    // TODO: Implement in a different way
    // if (decode) {
    //   purl = URLDecoder.decode(component.getPurl(), StandardCharsets.UTF_8.name());
    // }
    return PackageRef.builder().purl(component.getPurl()).build();
  }
}
